...
async function verifyReleaseConfigs() {
  ...
  try {
    ...
    if (mainConfig.plugins) {
      mainNpmPlugin = mainConfig.plugins.find(
      ...
      if (mainNpmPlugin) {
        ...
        console.log(
          '⚠️ This may be intentional, but verify that main can publish to NPM if needed',
        );
      }
    } else {
    ...
    if (devConfig.plugins) {
      devNpmPlugin = devConfig.plugins.find(
        (plugin) => Array.isArray(plugin) && plugin[0] === '@semantic-release/npm',
      );
    }
    ...
    if (devNpmPlugin) {
      console.log(
        '⚠️ NPM Plugin found in dev branch config:',
        JSON.stringify(devNpmPlugin[1], null, 2),
      );
    ...
    if (
      devConfig.plugins &&
      (devConfig.verifyConditions || devConfig.prepare || devConfig.publish)
    ) {
      duplicatedSections.push(
        'Dev config: Using both plugins array and step-specific configurations can cause conflicts',
      );
    }
    ...
    if (
      mainConfig.plugins &&
      (mainConfig.verifyConditions || mainConfig.prepare || mainConfig.publish)
    ) {
      duplicatedSections.push(
        'Main config: Using both plugins array and step-specific configurations can cause conflicts',
      );
    }
    ...
    console.log(
      '✓ For dev branch, use: npx semantic-release --no-ci --no-npm --extends $(pwd)/.release-dev.json',
    );
    console.log(
      '  This ensures that npm tokens are not validated and config is completely replaced',
    );
    ...
    console.log(
      '  GITHUB_TOKEN=dummy npx semantic-release --dry-run --no-ci --no-npm --extends $(pwd)/.release-dev.json',
    );
